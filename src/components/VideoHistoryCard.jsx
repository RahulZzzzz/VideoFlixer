...
function VideoHistoryCard({video}) {
    ...
    const deleteHandler = ()=>{
        // console.log('data');
        axios.get(`${url}/users/remove-from-history/${video._id}`, { withCredentials: true })
            .then((res)=>{
                console.log(res);
                setIsDeleted(true)
            })
    ...
    >
        ...
        <div className=' flex flex-col gap-4 items-start ' >
            ...
            <div className={` flex flex-row flex-wrap items-center gap-3 text-[16px] ${(theme=='dark')?'text-neutral-300':'text-neutral-800'}`}>
                <Link to={`/profile/${video.owner._id}`} className=' flex flex-row items-center gap-1'>
                    <img className=' w-[1.5rem] aspect-square rounded-full' src={video.owner.avatar} alt="" />
                    <div className=' hover:underline'>{video.owner.fullName}</div>
                </Link>
                <div className=' flex'>
        ...
        <div className="absolute top-0 right-0 mt-2 mr-2">
          ...
          {(isDropdownOpen ) && (
            <div className="absolute right-0 mt-2 bg-neutral-800 border border-neutral-600 rounded-md shadow-lg">
              ...
              <DialogBox
                title={"Remove Video from History"}
